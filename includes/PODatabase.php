...
class PODatabase {
    ...
    public function __construct()
    {
        try
        {
            //$this->podo = new PDO(PODB_DSN, PODB_USERNAME, PODB_PASSWORD, array(PDO::MYSQL_ATTR_INIT_COMMAND => "SET NAMES '" . PODB_CHARACSET . "';"));

            $hostname = "172.16.10.120";
            $dbname = "sgdb_sg3";
            $username = "satia";
            $pw = "2213200";
            $this->podo = new PDO ("dblib:charset=UTF-8;host=$hostname;dbname=$dbname","$username","$pw");
            //$this->podo->exec("SET CHARACTER SET " . PDB_CHARACSET);
            $this->podo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
            //$this->podo->query("set names " . PDB_CHARACSET);
    ...
    {
        $this->podo->setAttribute(PDO::ATTR_AUTOCOMMIT, 0);
        $this->podo->beginTransaction();
    }
    /**
    ...
    {
        $this->podo->commit();
        $this->podo->setAttribute(PDO::ATTR_AUTOCOMMIT, 1);
    }
    /**
    ...
    public function fetch_custom($query)
    {
        $sel = $this->podo->prepare($query);
        $sel->execute();
        $sel->setFetchMode(PDO::FETCH_OBJ);
        return $sel;
    }
    ...
    {
        $nilai=array($val);
        $sel = $this->podo->prepare("SELECT * FROM $table WHERE $col=?");
        $sel->execute($nilai);
        $sel->setFetchMode( PDO::FETCH_OBJ );
        $obj = $sel->fetch();
        return $obj;
    ...
    {
        $sel = $this->podo->prepare("SELECT * FROM $table");
        $sel->execute();
        $sel->setFetchMode( PDO::FETCH_OBJ );
        return $sel;
    }
    ...
    public function fetch_col($table,$dat)
    {
        ...
        $col=implode(', ', $cols);
        $sel = $this->podo->prepare("SELECT $col from $table");
        $sel->execute();
        $sel->setFetchMode( PDO::FETCH_OBJ );
        return $sel;
    }
    ...
    public function fetch_multi_row($table,$col,$where)
    {
        ...
        if ($jum>1) {
            $im=implode('? and  ', $mark);
            $sel = $this->podo->prepare("SELECT $colum from $table WHERE $im");
        } else {
            $im=implode('', $mark);
            $sel = $this->podo->prepare("SELECT $colum from $table WHERE $im");
        }
    ...
    public function fetch_first_row($table,$col,$where)
    {
        $data = array_values( $where );
        //grab keys
        $cols=array_keys($where);
    ...
    public function fetch_multi_row_order($table,$col,$where,$order,$index)
    {
        ...
        if ($jum>1) {
            $im=implode('? and  ', $mark);
            $sel = $this->podo->prepare("SELECT $colum from $table WHERE $im ORDER BY $index $order");
        } else {
            $im=implode('', $mark);
            $sel = $this->podo->prepare("SELECT $colum from $table WHERE $im ORDER BY $index $order");
        }
    ...
    public function check_exist($table,$dat) {
        ...
        if ($jum>1) {
            $im=implode(' and  ', $mark);
            $sel = $this->podo->prepare("SELECT $col from $table WHERE $im");
        } else {
            $im=implode('', $mark);
            $sel = $this->podo->prepare("SELECT $col from $table WHERE $im");
        }
    ...
    public function insert($table,$dat) {
        ...
        $im=implode(', ', $mark);
        $ins = $this->podo->prepare("INSERT INTO $table ($col) values ($im)");
        $ins->execute( $data );
    }
    /**
     * update record
    ...
    public function update($table,$dat,$id,$val) {
        ...
        $im=implode(', ', $mark);
        $ins = $this->podo->prepare("UPDATE $table SET $im where $id=?");
        $ins->execute( $data );
    }
    /**
     * delete record
    ...
    public function delete( $table, $where,$id ) {
        $data = array( $id );
        $sel = $this->podo->prepare("Delete from $table where $where=?" );
        $sel->execute( $data );
    }
